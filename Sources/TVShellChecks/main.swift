...
struct TVShellChecks {
    ...
    static func checkOfficialAnimeSourceStateAndAniGamerCatalog() throws {
        ...
        try expect(AniGamerRemoteBridge.action(for: .left) == .key(code: 123, characters: 0xF702), "aniGamer left seeks through the official player")
        try expect(AniGamerRemoteBridge.action(for: .right) == .key(code: 124, characters: 0xF703), "aniGamer right seeks through the official player")
        ...
        try expect(AniGamerRemoteBridge.action(for: .select) == .key(code: 49, characters: 32), "aniGamer OK activates the official player")
        try expect(AniGamerRemoteBridge.action(for: .playPause) == .key(code: 40, characters: 107), "aniGamer play/pause sends the official player shortcut")
        try expect(AniGamerRemoteBridge.action(for: .menu) == .key(code: 3, characters: 102), "aniGamer Menu toggles fullscreen")
        try expect(AniGamerRemoteBridge.domKey(for: .left) == .init(key: "ArrowLeft", code: "ArrowLeft"), "aniGamer left reaches the official player's DOM shortcut")
        try expect(AniGamerRemoteBridge.domKey(for: .right) == .init(key: "ArrowRight", code: "ArrowRight"), "aniGamer right reaches the official player's DOM shortcut")
        try expect(AniGamerRemoteBridge.domKey(for: .menu) == .init(key: "f", code: "KeyF"), "aniGamer Menu reaches the official fullscreen shortcut")
    ...
    static func checkTorrentPlaybackEngine() throws {
        ...
        let sparseSample = downloadDirectory.appendingPathComponent("第03話.mp4")
        ...
        FileManager.default.createFile(atPath: sparseSample.path, contents: Data(repeating: 1, count: 1_024))
        let sparseHandle = try FileHandle(forWritingTo: sparseSample)
        try sparseHandle.truncate(atOffset: 64 * 1_048_576)
        ...
        let physicalBytes = engine.fileBufferedBytes(sample)
            + engine.fileBufferedBytes(secondSample)
            + engine.fileBufferedBytes(sparseSample)
        try expect(engine.downloadProgress(in: downloadDirectory).downloadedBytes == physicalBytes, "torrent playback reports physical bytes instead of a sparse file's logical length")
    ...
    static func checkBigScreenViewsStayScrollableAndWindowIsResizable() throws {
        ...
        try expect(launcher.contains("ScrollViewReader { dockScrollProxy in"), "launcher dock owns a horizontal scroll reader")
        try expect(launcher.contains("onChange(of: focusedAppID)"), "launcher dock follows remote focus changes")
        try expect(launcher.contains("dockScrollProxy.scrollTo(\"tvos-dock-app-\\(id.uuidString)\", anchor: .center)"), "launcher dock centers the focused app tile")
