...
MacTV / TVShell 是一個跑在 macOS 上、以 Apple TV / tvOS 大螢幕體驗為目標的 SwiftUI 應用殼。它提供可用遙控器操作的 App Launcher、內建瀏覽器、YouTube、Bilibili 番劇、影片播放器、動漫入口、設定、App 管理、動漫來源管理與遙控器學習頁。
...
- Bilibili 作為獨立 App，參考 blbl 的 PGC 資料流，使用原生大螢幕 UI 搜尋番劇、顯示詳情、選集並解析 `pgc/player/web/playurl` 播放公開可看的集數。
...
- Bilibili 番劇仍受 B 站 API、登入、會員、地區與版權限制影響；App 不繞過限制，只會顯示 API 回傳錯誤。
